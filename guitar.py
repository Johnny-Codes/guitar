...
def fretboard_memorization():
    fretstrings = open("json_data/fret_strings.json")
...
def get_triad():
    triads = open("json_data/triads.json")
...
def triads_in_major_scale():
    triads = open("json_data/triads.json")
    triads = json.load(triads)
    major_scales = open("json_data/major_scale.json")
...
def parallel_keys():
    parallel_keys = open("json_data/parallel_keys.json")
    parallel_keys = json.load(parallel_keys)
    a = input("What key do you want to use? ")
    parallel_key = parallel_keys[a]
    print(
        "The parallel key for {a} is {parallel_key}".format(
            a=a, parallel_key=parallel_key
        )
    )
...
def minor_scale(x):
    minor_scales = open("json_data/minor_scale.json")
    minor_scales = json.load(minor_scales)
...
def ionian_scale(x):
    ionian_scales = open("json_data/ionian_scale.json")
    ionian_scales = json.load(ionian_scales)
    x = x[0] + " Ionian"
